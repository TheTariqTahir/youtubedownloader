...
class Main(MDApp):
    ...
    def build(self):
        ...
        self.filename='Please enter Url'
    ...
    # ==========================   Get video
    def get_video(self, url, image, spinner, quality_btn, download_btn,file_name):
...
#@----------------------------------     Download Video
    ...
    def download_video(self, spinner):
        try:
            ...
            name_old = self.filename.replace('|', '').replace('/', '').replace("\\", '').replace(":", '').replace("?", '').replace("*", '').replace("<", '').replace(">", '').strip()
